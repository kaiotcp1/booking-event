...
export default async function Home({ searchParams }: Props) {
  ...
  return (
    <div className="flex flex-col items-center m-1 gap-1 md:flex-row md:items-start md:mx-5 md:gap-5 shadow-md bg-gray-800">
      <div className="flex flex-col gap-5 mt-5 shadow-md w-full h-screen">
        <Filters />
        {events.map((event) => (
          <div key={event._id} className="grid grid-cols-1 md:grid-cols-3 bg-gray-700 p-5 md:p-0 rounded-sm shadow-2xl w-full" >
            <div className="col-span-1">
              <img src={event?.images[0]}
                ...
                height={250}
                width={250}
                className="w-full object-contain border rounded-l-sm shadow-md"
              />
            </div>
